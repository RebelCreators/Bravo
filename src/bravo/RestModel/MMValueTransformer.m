...
+ (RCServiceIOType)serviceTypeForClass:(Class)clazz {
    ...
    RCServiceIOType serviceType = RCServiceIOTypeVoid;
    
    if ([clazz conformsToProtocol:@protocol(MMEnumAttributeContainer)]) {
        serviceType = RCServiceIOTypeEnum;
    } else if ([clazz isSubclassOfClass:MMModel.class]) {
        serviceType = RCServiceIOTypeMagnetNode;
    }
    return serviceType;
}
...
+ (instancetype)transformerForType:(RCServiceIOType)type clazz:(Class)clazz {

    MMValueTransformer *valueTransformer;

    switch (type) {
        case RCServiceIOTypeVoid:break;
        case RCServiceIOTypeString:break;
        case RCServiceIOTypeEnum:{
            valueTransformer = [self enumTransformerForContainerClass:clazz];
            break;
        }
        case RCServiceIOTypeBoolean:break;
        case RCServiceIOTypeChar:break;
        case RCServiceIOTypeUnichar:break;
        case RCServiceIOTypeShort:break;
        case RCServiceIOTypeInteger:break;
        case RCServiceIOTypeLongLong:break;
        case RCServiceIOTypeFloat:break;
        case RCServiceIOTypeDouble:break;
        case RCServiceIOTypeBigDecimal:
        case RCServiceIOTypeBigInteger:{
            valueTransformer = [MMValueTransformer bigDecimalTransformer];
            break;
        }
        case RCServiceIOTypeDate:{
            valueTransformer = [MMValueTransformer dateTransformer];
            break;
        }
        case RCServiceIOTypeUri:{
            valueTransformer = [MMValueTransformer urlTransformer];
            break;
        }
        case RCServiceIOTypeArray:{
            RCServiceIOType serviceType = [MMValueTransformer serviceTypeForClass:clazz];
            valueTransformer = [self listTransformerForType:serviceType clazz:clazz];
            break;
        }
        case RCServiceIOTypeDictionary:{
            RCServiceIOType serviceType = [MMValueTransformer serviceTypeForClass:clazz];
            valueTransformer = [self mapTransformerForType:serviceType clazz:clazz];
            break;
        }
        case RCServiceIOTypeData:break;
        case RCServiceIOTypeBytes:break;
        case RCServiceIOTypeMagnetNode:{
